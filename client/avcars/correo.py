...
def _es_fallo_de_certificado(exc: BaseException) -> bool:
    """¿Este error es de certificado, aunque venga envuelto?

    `urlopen` no deja salir el `ssl.SSLError` tal cual: lo mete dentro de un
    `URLError`, en `.reason`. Un `except ssl.SSLError` alrededor de `urlopen`
    **no salta nunca**, y el aviso del antivirus se perdía por el camino.
    """
    return isinstance(exc, ssl.SSLError) or isinstance(
        getattr(exc, "reason", None), ssl.SSLError
    )


def _fallo_de_certificado(exc: BaseException) -> CorreoNoEnviado:
    """El error de certificado casi siempre es un antivirus, no el proveedor.

    Norton (y compañía) se meten en medio del tráfico cifrado y presentan un
    certificado propio: el nombre del emisor lo delata («generated by Norton
    Antivirus for SSL»). Pasa **solo en el equipo de desarrollo**; un servidor
    no lleva antivirus, así que allí no se ve. Costó un rato averiguarlo las
    dos veces, así que aquí queda dicho.
    """
    return CorreoNoEnviado(
        "No se pudo verificar el certificado del proveedor de correo. Suele "
        "ser un antivirus que inspecciona el tráfico cifrado (en el equipo de "
        "desarrollo era Norton); en el servidor esto no pasa. "
        f"Detalle: {exc}"
    )
...
def _token_de_acceso(cfg: Configuracion) -> str:
    ...
    try:
        ...
        if _es_fallo_de_certificado(exc):
            raise _fallo_de_certificado(exc) from exc
...
def _enviar_por_api(
    ...
    try:
        ...
        # El de certificado va aquí dentro y no en su propio `except`: urlopen
        # lo entrega envuelto en URLError, así que atraparlo suelto no vale.
        if _es_fallo_de_certificado(exc):
            raise _fallo_de_certificado(exc) from exc
